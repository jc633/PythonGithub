...
@function:自定义高亮标签，解决原标签{%highlight%}内容截取问题
@author: jxc
'''
...
class HighlightNode(template.Node):
    def __init__(self, text_block, query, html_tag=None, css_class=None, max_length=None, cut=None):
        self.text_block = template.Variable(text_block)
        self.query = template.Variable(query)
        self.html_tag = html_tag
        self.css_class = css_class
        self.max_length = max_length
        self.cut = cut  # 添加剪切属性,控制是否剪切高亮词的前面或后面内容
        ...
        if cut is not None:
            self.cut = template.Variable(cut)
    ...
    def render(self, context):
        ...
        if self.cut is not None:
            kwargs['cut'] = self.max_length.resolve(context)
        ...
        if hasattr(settings, 'HAYSTACK_CUSTOM_HIGHLIGHTER') and settings.HAYSTACK_CUSTOM_HIGHLIGHTER:
            ...
            try:
                ...
                highlighter_path, highlighter_classname = '.'.join(
                    path_bits[:-1]), path_bits[-1]
                highlighter_module = importlib.import_module(highlighter_path)
                highlighter_class = getattr(
                    highlighter_module, highlighter_classname)
            except (ImportError, AttributeError) as e:
                raise ImproperlyConfigured("The highlighter '%s' could not be imported: %s" % (
...
def myhighlight(parser, token):
    ...
    if len(bits) < 4:
        raise template.TemplateSyntaxError(
            u"'%s' tag requires an object and a query provided by 'with'." % tag_name)
